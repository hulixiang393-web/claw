...
| 5 | **续读记忆** | 每部作品显示"读到第45章/第3话/第6集"角标，点击直接续读（数据统一在书架）。**小说/漫画/视频都覆盖，本地与在线共用同一套记忆**：本地 epub 按文件路径、在线按作品 URL、本地视频按目录路径作 key（路径统一解析绝对路径，避免相对/绝对不一致导致记忆断链）；切回书架 Tab 自动刷新，读完回来立即显示最新进度 |
| 6 | 分组视图 | 本地 / 收藏 两个分组 |
...
点书架某作品卡片
  ...
  → 阅读器内翻章 → 更新续读记录
本地/在线合并卡（同书既收藏又已下载）：
  → 本地优先读本地；本地视频无进度时回退线上收藏的"看到第X集"定集
